  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const famRes = await axiosClient.get(
        ...
        else if (famRes && famRes.data && Array.isArray(famRes.data))
          famList = famRes.data;

        // Fetch patient self details
        const selfRes: any = await axiosClient.get(
        ...
        else if (relRes.isSuccess && Array.isArray(relRes.data))
          relList = relRes.data
            .filter((item: any) => item.isActive)
            .map((item: any) => ({
              masterDataId: item.masterDataId,
              ...
              <ScrollView style={{ maxHeight: 320 }}>
                {familyMembers.length === 0 ? (
                  ...
                  familyMembers.map((member) => (
                    <TouchableOpacity
                      key={
                        member.empRelationId === 0
                          ? "self"
                          : member.empRelationId
                      }
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                      ...
                      onPress={() => {
                        ...
                        if (
                          ...
                          (member.relation === "Self" ||
                            (member.relationName || "").trim().toLowerCase() ===
                              "self")
                        ) {
                          memberToSend = {
                            ...member,
                            empRelationId: member.patientId,
                          };
                        }
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        >
                          {member.empRelationId === 0
                            ? "Self"
                            : relationTypes.find(
                                (r) => r.masterDataId === member.relationId,
                              )?.name ||
                              member.relation ||
                              ""}{" "}
